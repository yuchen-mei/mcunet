...
PREVIOUS_LAYER = "InvRes14_pw_exp_bias_relu6"
...
def preprocess(WORK_DIR, ACT_PTH, WEIGHT_PTH, HALIDE_PTH):
    ...
    weights_npy_glb0 = weight_npy[0::4, :, :, :]
    weights_npy_glb1 = weight_npy[1::4, :, :, :]
    weights_npy_glb2 = weight_npy[2::4, :, :, :]
    weights_npy_glb3 = weight_npy[3::4, :, :, :]

    input_npy = numpy.zeros((input_npy_glb0.shape[0], input_npy_glb0.shape[1]+input_npy_glb1.shape[1], input_npy_glb0.shape[2], input_npy_glb0.shape[3]))
    input_npy[:, 0::2, :, :] = input_npy_glb0[:, :, :, :]
    input_npy[:, 1::2, :, :] = input_npy_glb1[:, :, :, :]

    weight_npy = numpy.zeros((weights_npy_glb0.shape[0]+weights_npy_glb1.shape[0], weights_npy_glb0.shape[1], weights_npy_glb0.shape[2], weights_npy_glb0.shape[3]))
    weight_npy[0::2, :, :, :] = weights_npy_glb0[:, :, :, :]
    weight_npy[1::2, :, :, :] = weights_npy_glb1[:, :, :, :]
...
def layer(WORK_DIR, ACT_PTH, WEIGHT_PTH, HALIDE_PTH):
    ...
    dw_conv = torch.nn.Conv2d(in_channels=192, out_channels=192, kernel_size=7, stride=2, padding=0, groups=192, bias=False)
