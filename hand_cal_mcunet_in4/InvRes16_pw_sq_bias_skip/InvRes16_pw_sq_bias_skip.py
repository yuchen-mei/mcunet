...
GOLD_PAD_LEFT = 0
GOLD_PAD_RIGHT = 6
INVRES_BLOCK_NAME = 'blocks_15_mobile_inverted_conv_point_linear'
ENTIRE_BLOCK_NAME = 'blocks_15'
PREVIOUS_LAYER = 'InvRes16_pw_sq'
SKIP_LAYER = 'InvRes15_pw_sq_bias_skip'
...
def preprocess(WORK_DIR, ACT_PTH, WEIGHT_PTH, HALIDE_PTH):
    ...
    skip_npy = numpy.load(f"../{SKIP_LAYER}/halide_data/hw_output.npy")
    ...
    skip_tensor = torch.from_numpy(skip_npy).bfloat16()
    ...
    processed_skip_tensor = skip_tensor
    ...
    hw_skip_stencil = processed_skip_tensor.to(torch.float32).numpy()
    ...
    # Save the processed skip to a .npy file
    numpy.save(HALIDE_PTH + 'hw_skip_stencil.npy', hw_skip_stencil)
    print(f"hw_skip_stencil shape: {hw_skip_stencil.shape}")
    print(HALIDE_PTH + "hw_skip_stencil.npy saved\n")
...
def layer(WORK_DIR, ACT_PTH, WEIGHT_PTH, HALIDE_PTH):
    ...
    skip_npy = numpy.load(HALIDE_PTH + 'hw_skip_stencil.npy')
    ...
    skip_tensor = torch.from_numpy(skip_npy).bfloat16().to('cuda')
    ...
    # Apply the bias+skip layer
    output = input_tensor + bias_tensor + skip_tensor
    ...
    gold_output_npy = numpy.load(ACT_PTH + f"{ENTIRE_BLOCK_NAME}/{ENTIRE_BLOCK_NAME}_out.npy")
...
def save_mat(WORK_DIR, ACT_PTH, WEIGHT_PTH, HALIDE_PTH):
    ...
    skip_npy = numpy.squeeze(numpy.load(HALIDE_PTH + 'hw_skip_stencil.npy'), axis=0)
    bias_npy = numpy.load(HALIDE_PTH + 'hw_bias_stencil.npy')
    ...
    convert_array_to_mat(skip_npy, 'hw_skip_stencil', HALIDE_PTH)
