...
INVRES_BLOCK_NAME = 'blocks_16_mobile_inverted_conv_inverted_bottleneck'
PREVIOUS_LAYER = 'InvRes16_pw_sq_bias_skip'
...
def layer(WORK_DIR, ACT_PTH, WEIGHT_PTH, HALIDE_PTH):
    ...
    pw_conv = torch.nn.Conv2d(in_channels=192, out_channels=768, kernel_size=1, stride=1, padding=0, bias=False)

    # Manually set the weights and biases of the convolutional layer
    pw_conv.weight = torch.nn.Parameter(weight_tensor)

    # Map layers to cuda
    pw_conv.to('cuda')

    # Apply the convolutional layers
    output = pw_conv(input_tensor.to('cuda'))
