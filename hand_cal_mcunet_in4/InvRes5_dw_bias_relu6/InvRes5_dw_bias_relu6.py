...
GOLD_PAD_LEFT = 0
GOLD_PAD_RIGHT = 4
INVRES_BLOCK_NAME = 'blocks_4_mobile_inverted_conv_depth_conv'
PREVIOUS_LAYER_1 = 'InvRes5_dw_1'
PREVIOUS_LAYER_2 = 'InvRes5_dw_2'
...
def preprocess(WORK_DIR, ACT_PTH, WEIGHT_PTH, HALIDE_PTH):
    ...
    input_npy_1 = numpy.load(f"../{PREVIOUS_LAYER_1}/halide_data/hw_output.npy")
    input_npy_2 = numpy.load(f"../{PREVIOUS_LAYER_2}/halide_data/hw_output.npy")
    input_npy = numpy.zeros((input_npy_1.shape[0], input_npy_1.shape[1]+input_npy_2.shape[1], input_npy_1.shape[2], input_npy_1.shape[3]))
    input_npy[:, 0::4, :, :] = input_npy_1[:, 0::2, :, :]
    input_npy[:, 1::4, :, :] = input_npy_1[:, 1::2, :, :]
    input_npy[:, 2::4, :, :] = input_npy_2[:, 0::2, :, :]
    input_npy[:, 3::4, :, :] = input_npy_2[:, 1::2, :, :]
    bias_npy = numpy.load(WEIGHT_PTH + f"{INVRES_BLOCK_NAME}_conv_bias_biases.npy")

    # Save the merged input to a .npy file
    numpy.save(HALIDE_PTH + '../hw_output_merged.npy', input_npy)
...
def layer(WORK_DIR, ACT_PTH, WEIGHT_PTH, HALIDE_PTH):
    ...
    # Create a relu6 layer and define output padding and trunc size
    ...
    # Apply the bias+relu6 layer
    output = relu6(input_tensor + bias_tensor)
    ...
    if out_of_tolerance[0].size > 0:
        ...
        # Print total number of values exceeding the tolerance
        print(f"\nTotal number of values exceeding the tolerance: {out_of_tolerance[0].size}\n")
